...
def pos_db(main_id, makepos_id = None, order_id = None):
    if id_col.find_one({'main_id': main_id}):
        ...
        if makepos_id and isinstance(makepos_id, str):
            ...
            makepos_price_list = id_col.find_one({'main_id': main_id})['makepos_price_list'].append(makepos_info['average'])
            makepos_id_list = id_col.find_one({'main_id': main_id})['makepos_id_list'].append(makepos_id)
            id_col.update_one({'main_id': main_id}, {'$set': {'pos_price': new_pos['pos_price'], 'makepos_price_list': makepos_price_list, 'makepos_id_list': makepos_id_list}}, {'$inc': {'trade_count': 1}})
    else:
        db_insert(main_id)
